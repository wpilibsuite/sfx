...
module SD
  class DataSourceSelector
    ...
    def initialize(core)
      ...
      HBox.setHgrow(@panes, Priority::ALWAYS) # silly this cant be in FXML
    ...
    def load_info_pane(url)
      if @old_url
        @name.text_property.unbind_bidirectional @old_url.name_property
        @host.text_property.unbind_bidirectional @old_url.host_property
      end
      @name.text_property.bind_bidirectional url.name_property
      @host.text_property.bind_bidirectional url.host_property
      annote = url.find_class.java_class.annotation(Java::dashfx.lib.controls.DesignableData.java_class)
      @type_info.text = "#{annote.name}\n#{annote.description}"
      @type_classname.text = url.class_name
      @old_url = url
    end

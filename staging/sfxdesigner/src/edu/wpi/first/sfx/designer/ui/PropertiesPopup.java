...
//    end
		public void setProperties(List<Property> props)
		{
			...
			this.allprops = props.stream().sorted((a, b) ->
			{
				...
				{
					if (a.getCategory().equals("Basic"))
					{
						return -1;
					}
					else
					{
						if (b.getCategory().equals("Basic"))
						{
							return 1;
						}
						else
			...
			if (bits != props && bits.size() != props.size())
			{
		...
		{
			prop_list.getChildren().clear();
			String lastType = "";
			for (Property prop : allprops)
			{
				if (!lastType.equals(prop.getCategory()))
				{
					addTitle(lastType = prop.getCategory());
				}
				showProp(prop);
			}
			show_decorators();
		...
		private void addTitle(String value)
		{
			...
			lbl.setFont(new Font("System Bold", 14));
			TitledFormPane.setExpand(lbl, true);
			prop_list.getChildren().add(lbl);
		}
